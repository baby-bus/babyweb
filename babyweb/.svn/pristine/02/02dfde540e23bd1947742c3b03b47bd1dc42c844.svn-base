...
class BlogAction extends CommonAction{
	...
	public function info(){
        $id = $_GET['id'];
        $minid = M('Website_blog')->where(" lang='fr' ")->min("id");
        $maxid = M('Website_blog')->where(" lang='fr' ")->max("id");
        $old_info = M('Website_blog')->where(" id<$id and lang='fr' ")->order('id desc')->find();
        $old_id = $old_info['id']; 
        if($id == $minid){
            $old_id = $id;
        }
        $new_info = M('Website_blog')->where(" id>$id and lang='fr' ")->order('id asc')->find();
        $new_id = $new_info['id'];
        if($id == $maxid){
            $new_id = $id;
        }
        $result = M('Website_blog')->where(" id=$id and lang='fr' ")->find();
