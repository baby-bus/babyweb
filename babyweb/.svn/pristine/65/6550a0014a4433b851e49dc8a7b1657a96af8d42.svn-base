...
//=====================================产品页相关Start==========================================
	 function scroll(class_name){
		 $(class_name).infinitescroll({
		        ...
		        itemSelector: ".border " , 
				loading: {
				finishedMsg: "데이터가 없습니다.",
	            img: "/Public/images/product/loading.gif",
	            msgText:"업로드중..."			//加载时显示的文字
				},
		  ...
		  if(keyword){
			  $.post(web_url+"/index.php/Product/search",{search_word:keyword},function(data){
				  ...
				  if(data.length > 1){
					  ...
					  $(".product_list div").remove();
					  $(".product_list").append("<div class='row'><div class='col-xs-12 text-center search_error'><img src='/Public/images/Public/search_error.png' /><P>새로운 도전을 하고있습니다.<a href='"+web_url+"/product'>기타 제품 </a>을 찾아보세요!</P></div></div>");	
				  }
			  },'html')
		  }
	  }
