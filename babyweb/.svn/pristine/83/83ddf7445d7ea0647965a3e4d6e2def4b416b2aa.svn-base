	  ...
	  function search_keyword(){
		  ...
		  if(keyword){
			  $.post(web_url+"/index.php/Product/search",{search_word:keyword},function(data){
				  product_label("Apps","Procurar Apps");
				  if(data.length > 1){
					  $("#navigation").attr('href',web_url+"index.php/Product/search/search_word="+keyword+"/page/2.html");
					  progoto(data,1);	
				  }else{
					  $(".product_list div").remove();
					  $(".product_list").append("<div class='row'><div class='col-xs-12 text-center search_error'><img src='/Public/images/Public/search_error.png' /><P>O que você quer encontrar nós também queremos desenvolver, veja <a href='"+web_url+"/product'>outras Apps!</a></P></div></div>");	
				  }
			  },'html')
		  }
	  }
