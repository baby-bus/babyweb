...
class BlogAction extends CommonAction{
	...
	public function getInfo(){
        ...
        foreach($res as &$v){
            ...
            $str = $v['content'];
            $v['match_p'] = mb_substr(strip_tags($str),0,300,'utf-8'); 
        }
        return $res;
	}
	...
	public function info(){
        $id = $_GET['id'];
        $minid = M('Website_blog')->where(" lang='en' ")->min("id");
        $maxid = M('Website_blog')->where(" lang='en' ")->max("id");
        $old_info = M('Website_blog')->where(" id<$id and lang='en' ")->order('id desc')->find();
        $old_id = $old_info['id']; 
        if($id == $minid){
            $old_id = $id;
        }
        $new_info = M('Website_blog')->where(" id>$id and lang='en' ")->order('id asc')->find();
        $new_id = $new_info['id'];
        if($id == $maxid){
            $new_id = $id;
        }
        $result = M('Website_blog')->where(" id=$id and lang='en' ")->find();
