...
//=====================================产品页相关Start==========================================
	 ...
	 function progoto(data,is_not_new){
	  ...
	  function search_keyword(){
		  ...
		  if(keyword){
			  $.post(web_url+"/index.php/Product/search",{search_word:keyword},function(data){
				  product_label("Applications","Recherche d'application");
				  if(data.length > 1){
					  $("#navigation").attr('href',web_url+"index.php/Product/search/search_word="+keyword+"/page/2.html");
					  progoto(data,1);	
				  }else{
					  $(".product_list div").remove();
					  $(".product_list").append("<div class='row'><div class='col-xs-12 text-center search_error'><img src='/Public/images/Public/search_error.png' /><P>Aucune application trouvée. Merci d'effectuer une <a href='"+web_url+"/product'>autre recherche</a>.</P></div></div>");	
				  }
			  },'html')
		  }
	  }
	  $(".educate li").click(function(){
		  ...
		  product_label("Applications",label,title);
