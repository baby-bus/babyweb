...
//=====================================产品页相关Start==========================================
	 function scroll(class_name){
		 $(class_name).infinitescroll({
		        ...
		        itemSelector: ".border " , 
				loading: {
				finishedMsg: "Keine weiteren Daten",
	            img: "/Public/images/product/loading.gif",
	            msgText:"Lädt gerade..."			//加载时显示的文字
				},
	  ...
	  function search_keyword(){
		  ...
		  if(keyword){
			  $.post(web_url+"/index.php/Product/search",{search_word:keyword},function(data){
				  product_label("Apps","Apps suchen");
				  if(data.length > 1){
					  $("#navigation").attr('href',web_url+"index.php/Product/search/search_word="+keyword+"/page/2.html");
					  progoto(data,1);	
				  }else{
					  $(".product_list div").remove();
					  $(".product_list").append("<div class='row'><div class='col-xs-12 text-center search_error'><img src='/Public/images/Public/search_error.png' /><P><a href='"+web_url+"/product'>Es wurden keine passenden Apps gefunden. Bitte sehen Sie sich nach einer anderen App um!</a></P></div></div>");	
				  }
			  },'html')
		  }
	  }
