$(document).ready(function() {
	...
	function showRequest(){
        ...
        if(ext!=".DOC"&&ext!=".DOCX"&&ext!=".WPS"&&ext!=".RAR"&&ext!=".ZIP"){ 
        	toastr.error("文件格式不正確！請上傳以下格式doc,docx,wps,rar,zip");
        	setInterval("$(\"#form_submit_check\").button('reset')",3000);
        	return false; 
        } 
        if($("#job_select option:selected").val()=="選擇申請崗位"){
        	toastr.error("請選擇崗位");
        	setInterval("$(\"#form_submit_check\").button('reset')",3000);
        	return false;
        }
	  ...
	  function search_keyword(){
		  ...
		  if(keyword){
			  $.post(web_url+"/index.php/Product/search",{search_word:keyword},function(data){
				  product_label("產品","搜索產品");
				  if(data.length > 1){
					  $("#navigation").attr('href',web_url+"index.php/Product/search/search_word="+keyword+"/page/2.html");
					  progoto(data,1);	
				  }else{
					  $(".product_list div").remove();
					  $(".product_list").append("<div class='row'><div class='col-xs-12 text-center search_error'><img src='/Public/images/Public/search_error.png' /><P>你要找的也是我們正要開發的，再看看<a href='"+web_url+"/product'>其他的產品</a>吧！</P></div></div>");	
				  }
			  },'html')
		  }
	  }
