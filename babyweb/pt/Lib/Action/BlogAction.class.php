...
class BlogAction extends CommonAction{
	...
	public function getInfo(){
        ...
        $where = "review=1 and lang='pt' ";
		$res =  $this->model->where($where)->order('id desc')->page("$page,3")->select();
        foreach($res as &$v){
	...
	public function info(){
        $id = $_GET['id'];
        $minid = $this->model->where(" lang='pt' ")->min("id");
        $maxid = $this->model->where(" lang='pt' ")->max("id");
        $old_info = $this->model->where(" id<$id and lang='pt' ")->order('id desc')->find();
        $old_id = $old_info['id'];
        if($id == $minid){
            $old_id = $id;
        }
        $new_info = $this->model->where(" id>$id and lang='pt' ")->order('id asc')->find();
        $new_id = $new_info['id'];
        if($id == $maxid){
            $new_id = $id;
        }
        $result = $this->model->where(" id=$id and lang='pt' ")->find();
